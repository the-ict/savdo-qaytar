      ...
      <div className="custom-container">
        <div className="flex flex-col md:row items-center justify-between mb-16 gap-6">
          <div className="max-w-xl text-center md:text-left">
            <h2 className="text-4xl font-bold mb-4">
              Treyderlar biz haqimizda
            </h2>
            <p className="text-muted-foreground">
              Minglab foydalanuvchilarimiz o\'z muvaffaqiyat hikoyalari bilan
              bo\'lishmoqda.
            </p>
          </div>
          <div className="flex gap-4">
            <div className="glass size-12 rounded-full flex items-center justify-center hover:bg-primary/10 cursor-pointer transition-colors">
              &lt;
            </div>
            <div className="glass size-12 rounded-full flex items-center justify-center hover:bg-primary/10 cursor-pointer transition-colors">
              &gt;
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {testimonials.map((t, idx) => (
            <div
              key={idx}
              className="glass p-8 rounded-[2rem] relative group border-primary/5 hover:border-primary/20 transition-all"
            >
              ...
              <div className="flex gap-1 mb-6">
                {[...Array(5)].map((_, i) => (
                  <Star
                    ...
                    className={cn(
                      'size-4',
                      i < t.rating ? 'fill-primary text-primary' : 'text-muted',
                    )}
                  />
                ))}
              </div>
